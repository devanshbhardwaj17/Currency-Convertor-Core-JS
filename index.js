...
function create(uniqId,textTitle,defaultvalue){
    let box=document.createElement("div")
    let inputbox=document.createElement("input")
    let countryList= document.createElement("select")
    let title=document.createElement("label")
    Object.entries(obj).map(([code, detail])=>{
    ...
    box.id="box"
    box.className=uniqId
    inputbox.type="number"
    inputbox.placeholder="Currency"
    inputbox.className="inputField"
    inputbox.id=`${uniqId}in`
    inputbox.value=0
    countryList.className="ctrList"
    countryList.id=`${uniqId}ctr`
    title.id="title"
    title.innerHTML=textTitle


    document.querySelector(`#main_div`).appendChild(box)
    box.appendChild(title)
    box.appendChild(document.createElement("br"))
    box.appendChild(inputbox)
    box.appendChild(countryList)
    
}
create("First","From","USD")
create("Second","To","INR")

let btnSection=document.createElement("div")
btnSection.id="btnSec"
...
document.querySelector(`#main_div`).appendChild(btnSection)
...
async function cal()
{
    const amount=document.querySelector(`#Firstin`).value
    let convamount=document.querySelector(`#Secondin`)
    let fromctry=document.querySelector(`#Firstctr`).value
    let toctry=document.querySelector(`#Secondctr`).value
    fromctry=fromctry.toUpperCase()
    toctry=toctry.toUpperCase()
    const url = `https://api.frankfurter.app/latest?amount=${amount}&from=${fromctry}&to=${toctry}`
    try {
        const response = await fetch(url);
        const data = await response.json();
        convamount.value=data.rates[toctry]
